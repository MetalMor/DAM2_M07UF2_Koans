...
namespace TheKoans
{
	...
	public class K030_AboutStrings : KoanHelper
	{
		...
		{
			var str = "Hola, Món";
            //Assert.AreEqual(typeof(FILL_ME_IN), str.GetType(), "Què li diu Arial a Serif? - Perdona, no ets el meu Tipus.");
		...
		public void SingleQuotedStringsAreNotStrings ()
		{
			...
			var str = 'H';
            //Assert.AreEqual (typeof(FILL_ME_IN), str.GetType (), "Un simple arbre no fa un bosc.");
		...
		public void VerbatimStringsCanHandleFlexibleQuoting ()
		{
			...
			Assert.AreEqual (strA.Equals(strB), strA.Equals (strB), "Ja us hem dit tres vegades que els Verbatim Strings poden tractar les cometes simples i dobles... Bé, suposo que això és la que fa quatre.");
		}
...
sóc una
línia trencada";
			Assert.AreEqual (27, verbatimString.Length, "És una nova línia un o dos caràcters?");
			//El que creeu pel literal string haurà d'escapar els caràcters de nova línia.
			var literalString = "Jo" + Environment.NewLine + "sóc una" + Environment.NewLine + "línia trencada";
...
línia trencada";
		...
		public void YouCanGetASingleCharacterFromAString ()
		{
			...
			Assert.AreEqual ('l', str [5], "els arrays indexats a 0 sovint poden resultar confusos... compteu el 0, o no?");
		}
		...
		public void StringsCanBeSplit ()
		{
            ...
            CollectionAssert.AreEqual(new string[] { "Salsitxa", "Ou", "Formatge" }, words, "Identifica tots els elements dins l'string 'words'. Oh! I espavil... Que m'està entrant gana!");
		}
		...
		public void UnderstandingThePerformanceOfCheckingForEmptyStrings ()
		{
			...
			//Assert.AreEqual (true, string.Empty, "A vegades cal aturar-se i agafar-se una estona per sentir l'eficiència...");
		}
		...
		public void CaseMattersWhenComparingStrings ()
		{
			...
			Assert.AreEqual (false, strProper.Equals (strUpper), "No és el que dius... És el com ho dius!");
		}
